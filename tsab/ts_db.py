...
class TSDatabase:
    ...
    def _TSFilenameStemSQL(self, ts_alias: str) -> str:
        return (
            "LOWER(CASE "
    ...
    def _TSNormalizeCompanionStemSQL(self, ts_expression: str) -> str:
        ts_cases: list[str] = []
    ...
    def _TSCompanionStemSQL(self, ts_alias: str) -> str:
        return self._TSNormalizeCompanionStemSQL(self._TSFilenameStemSQL(ts_alias))

    def _TSCompanionImageExclusionClause(self, ts_outer_alias: str = "assets_view") -> str:
        ts_outer_stem = self._TSCompanionStemSQL(ts_outer_alias)
        ts_inner_stem = self._TSCompanionStemSQL("companion_view")
        return f"""(
    ...
    def _TSFilterIncludesType(self, ts_filters: dict[str, Any] | None, ts_type_key: str) -> bool:
    ...
    def _TSBuildQueryParts(
        ...
        if self._TSFilterIncludesType(ts_filters, "image"):
            ts_where_clauses.append(self._TSCompanionImageExclusionClause("assets_view"))
        ts_where_sql = f" WHERE {' AND '.join(ts_where_clauses)}" if ts_where_clauses else ""
    ...
    def TSQueryAssets(
        ...
        ts_from_sql, ts_where_sql, ts_parameters, ts_order_by_sql = self._TSBuildQueryParts(ts_search_text, ts_filters)
