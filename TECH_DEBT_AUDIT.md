...
3. **HIGH — RESOLVED** — Тесты с `aiohttp`/`send2trash`/`PIL` оборачивают опасные импорты `importlib.util.find_spec` + `@unittest.skipIf` на тест-классах. Ran 175 tests, 27 skipped, 0 errors.
...
7. **MED — RESOLVED** — `TSResolveCachePath` (`tsab/ts_storage.py`) теперь кидает `ValueError` для путей вне `ts_asset_browser_directory`; все callerы (`ts_preview`, `ts_asset_payload`, `ts_asset_processing`, `ts_runtime`) гасят эту ошибку без падения запроса.
...
|----|-----------|-----------|-----|--------|----------|--------------|
| F01 | **RESOLVED** Bug — runtime infinite loop | js/ts-artius-browser-3d-worker.js | Critical | S | Воркер шлёт `after_sort`/`after_id` и читает `next_cursor` из ответа; цикл выходит при `!has_more` или `next_cursor === null`. Покрыт characterization-тестом в `scripts/check_frontend_3d_characterization.mjs` (`tsRequestedURLs` walk). | — |
| F02 | **RESOLVED** Test debt — broken release check | tests/test_config_store.py | High | S | Тесты теперь читают `TS_DEFAULT_CONFIG["version"]` через локальную константу `TS_CURRENT_CONFIG_VERSION`. | — |
| F03 | **RESOLVED** Test debt — fragile imports | tests/test_3d_thumbnail.py, tests/test_delete_service.py, tests/test_image_prompt_metadata.py, tests/test_workflows.py, tests/test_runtime_*.py | High | M | Каждый файл проверяет `importlib.util.find_spec` для нужных модулей и оборачивает импорты + decoratнул класс `@unittest.skipIf`. `Ran 175 tests in 0.455s, OK (skipped=27)`. | — |
...
| F07 | **RESOLVED** Path-traversal latent | tsab/ts_storage.py | Medium | S | После `resolve()` проверяем `relative_to(self.ts_asset_browser_directory)` и кидаем `ValueError` с verbose-логом `storage.cache_path.outside_root`. Все callerы (`TSPurgePreview`, `TSBuildAssetCard`, `_TSPreviewFileReady`, `TSBuildPreviewResponse`, `_TSResolvePreviewFilePath`) обрабатывают ошибку: считают что preview отсутствует/падают на placeholder. Регрессионные тесты — `tests/test_storage_paths.py::test_resolve_cache_path_*`. | — |
